...
export default function TablesForEntity() {
  const [selectedCertificate, setSelectedCertificate] = React.useState('TATA CLiQ - Customer Loyalty A1');
  const [pan, setPan] = React.useState('');
  function issueCertificate() {
    alert(`Certificate of type ${selectedCertificate} has been issued ${pan} individual.`);
  }
  return (
    <>
      <div className="flex flex-wrap mt-4">
        <div className="w-full mb-12 px-4">
          <CardTableForEntityForCertificate setSelectedCertificate={setSelectedCertificate} />
            <div>
              Issue a "<span className="font-bold">{selectedCertificate}</span>" certificate to individual with{" "}
              <input
